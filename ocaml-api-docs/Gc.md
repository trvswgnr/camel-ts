# Module [Gc](type_Gc.html)


```
module Gc: sig [..](Gc.html) end
```
...
|  | `live_words : `int`;` | `(*` | Number of words of live data in the major heap, including the header  words. Note that "live" words refers to every word in the major heap that isn't  currently known to be collectable, which includes words that have become  unreachable by the program after the start of the previous gc cycle.  It is typically much simpler and more predictable to call  [`Gc.full_major`](Gc.html#VALfull_major) (or [`Gc.compact`](Gc.html#VALcompact)) then computing gc stats, as then  "live" words has the simple meaning of "reachable by the program". One  caveat is that a single call to [`Gc.full_major`](Gc.html#VALfull_major) will not reclaim values  that have a finaliser from [`Gc.finalise`](Gc.html#VALfinalise) (this does not apply to  [`Gc.finalise_last`](Gc.html#VALfinalise_last)). If this caveat matters, simply call  [`Gc.full_major`](Gc.html#VALfull_major) twice instead of once. | `*)` |
|  | `live_blocks : `int`;` | `(*` | Number of live blocks in the major heap. See `live_words` for a caveat about what "live" means. | `*)` |
...
|  | `stack_size : `int`;` | `(*` | Current size of the stack, in words.  This metrics is currently not available in OCaml 5: the field value is  always `0`.  * **Since** 3.12 | `*)` |
|  | `forced_major_collections : `int`;` | `(*` | Number of forced full major collections completed since the program  was started.  * **Since** 4.12 | `*)` |
...
|  | `minor_heap_size : `int`;` | `(*` | The size (in words) of the minor heap. Changing  this parameter will trigger a minor collection. The total size of the  minor heap used by this program is the sum of the heap sizes of the  active domains. Default: 256k. | `*)` |
| --- | --- | --- | --- | --- |
...
```
val stat : unit -> [stat](Gc.html#TYPEstat)
```
