...
import * as Belt__Belt_MapInt from "../melange.belt/belt_MapInt.mjs";
import * as Belt__Belt_MapString from "../melange.belt/belt_MapString.mjs";
...
function get_method_label(table, name) {
  var x = Belt__Belt_MapString.getUndefined(table.methods_by_name, name);
  if (x !== undefined) {
    return x;
  }
  var label = new_method(table);
  table.methods_by_name = Belt__Belt_MapString.set(table.methods_by_name, name, label);
  table.methods_by_label = Belt__Belt_MapInt.set(table.methods_by_label, label, true);
  return label;
}
...
function set_method(table, label, element) {
  ...
  if (Belt__Belt_MapInt.getExn(table.methods_by_label, label)) {
...
function narrow(table, vars, virt_meths, concr_meths) {
  ...
  table.vars = Belt__Belt_MapString.reduceU(table.vars, undefined, (function (tvars, lab, info) {
          if (Stdlib__List.mem(lab, vars$1)) {
            return Belt__Belt_MapString.set(tvars, lab, info);
          } else {
            return tvars;
          }
        }));
  ...
  Stdlib__List.iter2((function (met, label) {
          by_name.contents = Belt__Belt_MapString.set(by_name.contents, met, label);
          by_label.contents = Belt__Belt_MapInt.set(by_label.contents, label, Belt__Belt_MapInt.getWithDefault(table.methods_by_label, label, true));
        }), concr_meths$1, concr_meth_labs);
  Stdlib__List.iter2((function (met, label) {
          by_name.contents = Belt__Belt_MapString.set(by_name.contents, met, label);
          by_label.contents = Belt__Belt_MapInt.set(by_label.contents, label, false);
...
function widen(table) {
  ...
  table.vars = Stdlib__List.fold_left((function (s, v) {
          return Belt__Belt_MapString.set(s, v, Belt__Belt_MapString.getExn(table.vars, v));
        }), match[3], match[5]);
...
function new_variable(table, name) {
  var x = Belt__Belt_MapString.getUndefined(table.vars, name);
  if (x !== undefined) {
    return x;
  }
  var index = new_slot(table);
  if (name !== "") {
    table.vars = Belt__Belt_MapString.set(table.vars, name, index);
  }
  return index;
}
...
function get_variable(table, name) {
  try {
    return Belt__Belt_MapString.getExn(table.vars, name);
  }
  catch (raw_exn){
    var exn = Caml_js_exceptions.internalToOCamlException(raw_exn);
    if (exn.RE_EXN_ID === Stdlib.Not_found) {
...
function create_table(public_methods) {
  ...
  Stdlib__Array.iteri((function (i, met) {
          var lab = (i << 1) + 2 | 0;
          table.methods_by_name = Belt__Belt_MapString.set(table.methods_by_name, met, lab);
          table.methods_by_label = Belt__Belt_MapInt.set(table.methods_by_label, lab, true);
        }), public_methods);
  return table;
}
